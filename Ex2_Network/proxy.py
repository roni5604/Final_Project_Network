...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start (1)
        # Tell the proxy on which port to listen to
        # Ready to listen up to 3 requests
        # proxy address is a tuple and contains two elements host and port
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # * Fill in start (3)
            # Getting the data from the socket with the max data of 8192
            # the max amount according to exercise is 8180
            ...
            try:
                ...
                # * Fill in start (4)
                #  After getting the response from the server the proxy send it back to the client
